...
/*
	...
	* The SMC is an Intel 8051 microcontroller inside the Southbridge, it handles low-level system tasks, such as the
	Power On Reset sequence, UART, Clock, DVD tray state, Tilt Status, IR Receiver, Temps, etc...

	Since emulating an 8051 core is just adding overhead to the emulator as of now i'm chosing just to do HLE for this. 
*/
	...
	namespace PCIDev
	{
		namespace SMC
		{
			...
			enum SMC_FIFO_CMD
			{
				...
				SMC_SET_FAN_SPEED_GPU	= 0x94, // Not present on Slims, not used/respected on newer fat.
			...
			enum SMC_PWR_REAS 
			{
				...
				SMC_PWR_REAS_REMOPWR		= 0x20, // XSS 2 power button on 3rd party remote/xbox universal remote
				...
				SMC_PWR_REAS_WIRELESS		= 0x55, // XSS 8 wireless controller middle button/start button pushed to 
													// power on controller and console
				SMC_PWR_REAS_WIRED_F1		= 0x56, // XSS 9 wired guide button; fat front top USB port, slim front 
													// left USB port
				SMC_PWR_REAS_WIRED_F2		= 0x57, // XSS A wired guide button; fat front botton USB port, slim front
													// right USB port
			...
			// slim with wired controller when horozontal, 3 back usb ports top to bottom 0x59, 0x58, 0x5A front 
			// left 0x56, right 0x57.
			// slim with wireless controller w/pnc when horozontal, 3 back usb ports top to bottom 0x55, 0x58, 
			// 0x5A front left 0x56, right 0x57.
			...
			// 1. The SMC PCI State (SMC_PCI_STATE): This is what the system sees/has R/W access trough the PCI Bus.
			// 2. The Inner State (SMC_CORE_STATE): tracking config settings like DVD Tray State, Current temps, 
			// Tilt status, etc...
			
			// SMC PCI State: 255 Bytes long.
			// There are some registers known, and some that aren't atm, so we'll be adding those later on.
