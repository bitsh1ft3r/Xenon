		...
		case ATA_REG_DATA:
			...
			{	
				...
				byteCount = (byteCount < ataDeviceState.readBuffer.capacity() ? 
					byteCount : ataDeviceState.readBuffer.capacity());
				...
				ataDeviceState.readBuffer.erase(ataDeviceState.readBuffer.begin(), 
					ataDeviceState.readBuffer.begin() + byteCount);
				return;
			}
			break;
		default:
...
void HDD::ataCopyIdentifyDeviceData()
{
	...
	{
		memcpy(ataDeviceState.readBuffer.data() + buffPos, (u8*)&ataDeviceState.ataIdentifyData + buffPos, 1);
	}
}
