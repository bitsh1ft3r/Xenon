...
// address of command block registers and the base address of control block registers.
...
/*
    This register contains the current status of the drive. If the BSY bit is 0, the other bits 
    of the register contain valid information; otherwise the other bits do not contain valid information. 
    If this register is read by the host computer during a pending interrupt, the interrupt 
...
/*
    Bit 0 (ERR / CHK – Error / Check) is defined as ERR for all commands except for the 
    Packet and Service commands, for which this bit is defined as CHK.
*/
#define ATA_STATUS_ERR_CHK              0x1
/*
    Bit 3 (DRQ – Data Request) indicates by value 1 that the disk drive is ready to transfer data 
    between the host computer and the drive. After the computer writes a commmand code to the 
    Command register, the drive sets the BSY bit or the DRQ bit to 1 until command completion.
*/
#define ATA_STATUS_DRQ                  0x08
/*
    Bit 5 (DF – Device Fault) indicates by value 1 that a device fault has been detected.
*/
#define ATA_STATUS_DF                   0x10
/*
    Bit 6 (DRDY – Device Ready) is set to 1 to indicate that the disk drive accepts commands. 
    If the DRDY bit is 0, the drive will accept and attempt to execute the Device 
    Reset and Execute Device Diagnostic commands. Other commands will not be accepted, and the
    drive will set the ABRT bit in the Error register and the ERR/CHK bit 
    in the Status register, before resetting the BSY bit to indicate completion of the command.
*/
#define ATA_STATUS_DRDY                 0x40
/*
    Bit 7 (BSY – Busy) is set to 1 whenever the disk drive has control of the Command Block 
    registers. If the BSY bit is 1, a write to any Command Block register by the host computer 
    will be ignored by the drive. The BSY bit is cleared to 0 by the drive at command completion
    and after setting the DRQ status bit to 1 to indicate the device is ready to transfer data.
*/
...
/*
    This is a 32-bit register and is used for reading or writing the data during data transfers.
    This register shall be accessed for data transfers in PIO mode only when the DRQ bit of the 
...
/*
    This register contains the status of the last command executed by the disk drive or a 
    diagnostic code. At completion of any command except the Execute Device Diagnostic and
    Device Reset commands, the contents of this register are valid when the BSY and DRQ bits of 
    the Status register are cleared to 0 and the ERR/CHK bit in the same register is set to 1. At
    completion of an Execute Device Diagnostic or Device Reset command and after a hardware 
    or software reset, this register contains a diagnostic code.
    Except for bit 2 (ABRT), the meaning of other bits of the Error register varies depending on 
    the command that has been executed.
*/

/*
    Bit 2 (ABRT – Command Aborted) indicates by value 1 that the requested command 
    has been aborted because the command code or a command parameter is invalid, the 
    command is not implemented, or some other error has occurred.
*/
...
/*
    This register is used for selecting the disk drive. The register shall be written only 
    when the BSY and DRQ bits of the Status register are both 0. The contents of this register 
    are valid only when the BSY bit of the Status register is 0. Except the DEV bit, all other 
    bits of this register become a command parameter when the command code is written into the 
    Command register
...
/*
    Bit 6 (LBA) selects the sector addressing mode. Some commands require to set this bit 
    to 1 to select LBA addressing. If this bit is cleared to 0, the CHS addressing is selected
*/
...
/*
    This register contains the command code to be sent to the disk drive. Command execution begins immediately after the command code is written into the Command register. The contents of the Command Block registers become parameters of the command when this register is 
    written. Writing this register clears any pending interrupt condition.
*/
...
/*
    This register contains the same information as the Status register. The only difference 
    is that reading the Alternate Status register does not imply an interrupt acknowledgement or 
    clearing of the interrupt condition
*/
...
/*
    This register allows the host computer to perform a software reset of the disk drives and 
    to enable or disable the assertion of the INTRQ interrupt signal by the selected drive. When 
    the Device Control register is written, both drives respond to the write regardless of which
    drive is selected.
...
/*
    Bit 1 (nIEN – INTRQ Enable) enables by value 0 the assertion of the INTRQ interrupt 
    request signal by the disk drive.
*/
#define ATA_DEVICE_CONTROL_NIEN                 0x02
/*
    Bit 2 (SRST – Software Reset) is the software reset bit of the disk drives. If there are
    two daisy-chained drives, by setting this bit to 1 both drives are reset.
*/
#define ATA_DEVICE_CONTROL_SRST                 0x04
/*
    Bit 7 (HOB – High Order Byte) is defined only when the 48-bit LBA addressing is
    implemented. If this bit is set to 1, reading of the Features register, the Sector Count
    register, and the LBA address registers is performed from the “previous content” location. 
    If the HOB bit is set to 0, reading is performed from the “most recently written” location. 
    Writing to any Command Block register has the effect of resetting the
    HOB bit to 0.
*/
...
{
    /*
        ATAPI drives will set the ABRT bit in the Error register and will place the signature of
        ATAPI drives in the Interrupt Reason, LBA Low, Byte Count Low, and Byte Count High
        registers.
...
void ODD::processSCSICommand()
{
    ...
    case SCSIOP_READ:
        ...
        break;
    default:
        std::cout << "ATAPI: Unknown SCSI Command requested: 0x" << atapiState.scsiCBD.CDB12.OperationCode
            << std::endl;
    }
...
void ODD::doDMA()
{
    for (;;)
    {
        ...
        memcpy(&atapiState.dmaState, mainMemory->getPointerToAddress
        (atapiState.atapiRegs.dmaTableOffsetReg + 
            atapiState.dmaState.currentTableOffset), 8); // Each entry is 64 bit long
...
void ODD::Read(u64 readAddress, u64* data, u8 byteCount)
{
    ...
    if (atapiCommandReg < (pciConfigSpace.configSpaceHeader.BAR1 - pciConfigSpace.configSpaceHeader.BAR0))
    {
        ...
        case ATAPI_REG_STATUS:
            ...
            return;
        default:
            std::cout << "ATAPI: Unknown Command Register Block register being read, command"
    ...
    else
    {
        ...
        case ATAPI_DMA_REG_TABLE_OFFSET:
            ...
            break;
        default:
            std::cout << "ATAPI: Unknown Control Register Block register being read, command"
...
void ODD::Write(u64 writeAddress, u64 data, u8 byteCount)
{
    ...
    if (atapiCommandReg < (pciConfigSpace.configSpaceHeader.BAR1 - pciConfigSpace.configSpaceHeader.BAR0))
    {
        ...
        case ATAPI_REG_DATA:
            ...
            // Check if we're executing a SCSI command input and we have a full command
        ...
        case ATAPI_REG_COMMAND:
            ...
            case ATA_COMMAND_IDENTIFY_DEVICE:
                ...
                return;
            default:
                std::cout << "ATAPI: Unknown command, command code = 0x" << data << std::endl;
                break;
            }
            return;
        default:
            std::cout << "ATAPI: Unknown Command Register Block register being written, command"
                << "code = 0x" << atapiCommandReg << std::endl;
            break;
        }
    }
    else
    {
        ...
        case ATAPI_DMA_REG_TABLE_OFFSET:
            ...
            break;
        default:
            std::cout << "ATAPI: Unknown Control Register Block register being written, command"
                << "code = 0x" << atapiControlReg << std::endl;
            break;
        }
    }
