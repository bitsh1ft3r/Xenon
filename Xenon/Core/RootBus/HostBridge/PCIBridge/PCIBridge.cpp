...
PCIBridge::PCIBridge()
{
	...
	pciBridgeConfig.configSpaceHeader.prefetchableBaseUpper32Bits = pciBridgeConfigMap[0xA];
	pciBridgeConfig.configSpaceHeader.prefetchableLimitUpper32Bits = pciBridgeConfigMap[0xB];
	...
	pciBridgeConfig.configSpaceHeader.expansionROMBaseAddress = pciBridgeConfigMap[0xE];
...
{
	std::cout << "PCI Bus > New device attatched: " << device->GetDeviceName() << std::endl;
...
bool PCIBridge::Read(u64 readAddress, u64* data, u8 byteCount)
{
	...
	if (readAddress >= PCI_BRIDGE_BASE_ADDRESS && readAddress <= PCI_BRIDGE_BASE_END_ADDRESS)
	{
		switch (readAddress)
		{
		case 0xEA000000:
...
bool PCIBridge::Write(u64 writeAddress, u64 data, u8 byteCount)
{
	...
	if (writeAddress >= PCI_BRIDGE_BASE_ADDRESS && writeAddress <= PCI_BRIDGE_BASE_END_ADDRESS)
	{
		switch (writeAddress)
		{
		case 0xEA000000:
...
void PCIBridge::ConfigRead(u64 readAddress, u64* data, u8 byteCount)
{
	...
	default:
		std::cout << "PCI Config Space Read: Unknown device accessed: Dev 0x"
			<< configAddr.devNum << " Reg 0x" << configAddr.regOffset << std::endl;
		return;
		break;
	}
...
void PCIBridge::ConfigWrite(u64 writeAddress, u64 data, u8 byteCount)
{
	...
	default:
		std::cout << "PCI Config Space Write: Unknown device accessed: Dev 0x"
			<< configAddr.devNum << " Func 0x" << configAddr.functNum << " Reg 0x" << configAddr.regOffset << " data = 0x"
			<< data << std::endl;
		return;
		break;
	}
	...
	for (auto &device : connectedPCIDevices)
	{
		if (device->GetDeviceName() == currentDevName)
		{
			...
			std::cout << "PCI Bus -> Config write, device: " << currentDevName << " addr = 0x"
				<< configAddr.regOffset << " data = 0x" << data << std::endl;
