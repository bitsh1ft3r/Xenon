...
void PostBus::Read(u64 readAddress, u64* data, u8 byteCount)
{
	std::cout << "POST Bus: Warning, read attempt to post bus. Returning last bus output." << std::endl;
	*data = lastBusData;
	return;
}

void PostBus::Write(u64 writeAddress, u64 data, u8 byteCount)
{
    u64 postCode = 0;
    memcpy(&postCode, &data, byteCount);
    POST(postCode);
}

void PostBus::POST(u64 postCode)
{
